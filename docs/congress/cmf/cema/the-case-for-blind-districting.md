...
Blind Districting eliminates both problems by eliminating the judgment. The algorithm has no political preferences, cannot be lobbied, does not deadlock, and produces identical outputs from identical inputs. There is nothing left to manipulate and nothing left to litigate.
...
### "Algorithms cannot ensure fair minority representation"

A blind algorithm operates without reference to race, ethnicity, or language. Becker, Duchin, Gold, and Hirsch (2021) have shown that ensemble methods can miss the concerns the Voting Rights Act was built to address if the underlying model lacks the right demographic and legal structure. So the objection runs: an algorithm that cannot see a minority community cannot protect it, and a districting system indifferent to race will dilute the voting strength of the communities the Voting Rights Act exists to defend.

The factual premise is correct. The algorithm is blind to race by design. But the objection assumes that the protection of minority representation runs through the boundary-drawing step -- that someone must consciously draw a district to protect a community, and that an algorithm which declines to do so leaves that community unprotected. Under the Act's architecture, that assumption no longer holds, because the protection of minority representation does not run through boundary placement at all. It runs through three structural features that operate regardless of where any line falls.

The first is House expansion. The current House -- 435 members for roughly 330 million people, a ratio frozen since 1929 -- is among the least representative lower chambers in the democratic world. Under the cube root rule, the Act expands the House toward approximately 720 seats. A larger chamber means more seats per district, and more seats per district means more room for a cohesive community to place a candidate among the winners. The second is Proportional STAR in multi-member districts, which turns that wider opening into actual representation. A community constituting 20 to 30 percent of a multi-member district elects representatives in proportion to its support -- not because a mapmaker drew it a district, but because the counting method allocates seats proportionally. The third is blind districting itself, which removes the discretion through which a hostile mapmaker could otherwise crack or pack a community out of the representation the first two features would give it.

These three features protect cohesive minorities of every kind -- racial, linguistic, religious, socioeconomic, ideological. This is a broader guarantee than the Voting Rights Act ever offered. Section 2 reached discrimination against racial and language minorities; it had nothing to say to a political or economic minority submerged in a winner-take-all district. The Act's structure protects all of them, through the same mechanism, without asking a court to certify which groups qualify.

This matters now in a way it would not have a year ago. In *Louisiana v. Callais*, 608 U.S. ___ (2026), the Supreme Court reconstrued Section 2 to impose liability only where the circumstances support a strong inference of intentional racial discrimination -- returning the provision, in the words of the dissent, to "all but a dead letter" as applied to redistricting. The same decision held that a State may draw districts for partisan advantage as "a constitutionally permissible criterion that States may rely on as desired," even where partisan and racial lines coincide. The practical effect is that a State may now dilute a minority community's voting strength and defend the result as mere partisanship, and the principal federal tool built to stop it will not reach the conduct.

That outcome vindicates the Act's design rather than threatening it. A protection that depends on a results-based statutory test, enforced case by case, in front of a judiciary free to narrow it, is a protection held on sufferance. It is a beautifully cut crystal: precisely targeted, admirable in its craftsmanship, and dependent on favorable political and judicial weather for its continued existence. When the weather turned in *Callais*, the crystal cracked, and sixty years of vote-dilution protection contracted in a single decision. The dissent observed that Section 2 had always carried its own sunset -- its preconditions require present-day geographic concentration and present-day racially polarized voting, so it self-liquidates as conditions change and can be contested map by map even where it still applies. The Act takes the opposite approach. It lays down brick on a foundation that does not move: the Elections Clause, Congress's settled authority over the manner of congressional elections. On that foundation it builds a structure that produces the result -- proportional representation for cohesive communities -- without the vulnerability. The Act does not ask a court to protect minority voters. It builds a system in which their representation is a function of arithmetic, and arithmetic does not depend on who sits on the bench.

This is why the Act contains no Voting Rights Act compliance mechanism for congressional districting, and why earlier drafts that did were revised to remove it. A race-conscious adjustment to a race-blind map would, under the doctrine *Callais* applies, invite challenge as the very kind of intentional racial line-drawing the Court now treats as a constitutional violation -- so the carve-out's remedy would have become its own liability. More fundamentally, the carve-out was never the thing protecting minority representation; the structure was. Removing it does not weaken the guarantee. It clarifies where the guarantee actually lives.

The contrast with the status quo is worth stating directly. Under single-member plurality, a community of color constituting 30 percent of a region's population receives zero representatives unless a mapmaker deliberately draws a majority-minority district. Even when such districts are created, they pack the community into one seat while bleaching the surrounding districts -- a dynamic both parties have historically exploited, and one that trades influence everywhere else for a single guaranteed seat. Under Proportional STAR in an expanded House, that same community elects representation proportional to its numbers without any human drawing a line on its behalf, and without surrendering its influence in the districts around it. The representation is emergent from the structure, not dependent on the goodwill or legal obligation of whoever holds the redistricting pen, and not contingent on a statute the Court has now hollowed out.
...
- Proportional STAR ensures proportional representation regardless of boundary placement, but someone still has to draw the district lines, and under human-drawn maps, that discretion is the attack surface for manipulation.
...
Blind Districting addresses this problem at its source. An algorithm that processes only population data and state boundaries cannot draw safe seats because it has no information about partisan composition. And even if, by geometric accident, an algorithmically drawn district happened to favor one party heavily, Proportional STAR in multi-member districts ensures that the minority party's voters still elect representation proportional to their numbers. The combination eliminates both the intentional gerrymandering that the metrics literature documents and the unintentional geographic sorting that Chen and Rodden (2013) have shown produces substantial partisan bias even under neutral redistricting.
...
Each defense covers a different attack surface. Capturing appointments does not help if existing commissioners cannot be removed during redistricting. Starving the budget does not help if the default algorithm is self-executing -- the Clerk of the House applies shortest splitline directly from census data, and the maps draw themselves. And there is no Department of Justice districting role left to weaponize: the Act assigns no federal actor any authority to draw, adjust, or approve congressional boundaries outside the algorithm itself. The architecture is designed for a hostile political environment, not a cooperative one.

---

## What This Does and Does Not Reach

The Act governs congressional elections. Its authority comes from the Elections Clause, which gives Congress power over the manner of electing Representatives and Senators, and it reaches no further than that. The algorithm draws congressional districts; it does not touch the maps a state draws for its own legislature, and the Act claims no power to.

This boundary is a feature, not a hedge. The same machinery the Act builds for congressional districting -- the certified algorithm, the published source code, the independent verification, the public replication tools -- is available to any state that wants to adopt it for its own legislative maps. A state that finds the federal approach persuasive can take it up and produce congressional and state districts on the same neutral basis. The Act extends the offer; it does not compel acceptance.

A state that prefers a different approach for its own elections is free to take one. What a state does with its own legislative districts is governed by its own constitution, its own courts, and its own voters, and several state constitutions and state courts police districting more searchingly than current federal doctrine does. That is the federal structure working as intended: the people of each state answer for the maps that govern their state, and the consequences of those choices fall where the choices are made.

What a state cannot do, under the Act, is carry a manipulated map into the people's House. The composition of Congress is a national concern, and the Elections Clause commits its manner to national resolution. The Act exercises that authority to take congressional districting out of contention entirely -- not by trusting any state to draw fair lines, and not by trusting any court to police unfair ones, but by removing the discretion that makes the question contestable in the first place.
...
- **Threshold-as-bar framing retired (1 site, 2 instances):** In "Algorithms cannot ensure fair minority representation," the three-feature structural argument rewritten. "A larger chamber means smaller effective thresholds: the share of the vote a cohesive community must assemble to win a seat falls as district magnitude rises" replaced with "more seats per district, and more seats per district means more room for a cohesive community to place a candidate among the winners." "Which converts that lowered threshold into actual representation" replaced with "which turns that wider opening into actual representation." The surrounding argument (three features operating regardless of boundary placement) and the proportionality description ("A community constituting 20 to 30 percent... elects representatives in proportion to its support") preserved unchanged
...
- Rev 6.4 disposition: no content references Title II party recognition structure, Title V compensation specifics, or Section 704 effective-date tiers. The threshold framing was the sole finding. Terminology verified: "Proportional STAR" used throughout (no "STAR-PR" drift); "Allocated Score" at algorithm level only

**Revision 6.2**
- Series-alignment pass: filename advanced from 6-1 to 6-2 to match the current CEMA legislative-text baseline. No content change. Rev 6.2 added an affirmative full-field elections standard to the legislative text and split the primary prohibition into two independent bars (no state funding, no state enforcement); this document does not describe the primary-prohibition mechanics and required no content change. VRA content confirmed current: this document was the anchor for the post-*Callais* structural reframe at Rev 6.1, carries no stale Section 2 carve-out language, and its retained references to minority representation and the *Callais* analysis remain accurate. Verified against both the VRA-residue and full-field/primary trigger sets
...
- **VRA counterargument section retitled and rewritten:** "Algorithms cannot handle VRA compliance" retitled to "Algorithms cannot ensure fair minority representation," reframing the section around the underlying concern (minority representation) rather than the proxy (statutory compliance). The prior answer -- which relied on the now-removed bounded VRA adjustment mechanism (DOJ review, minimum adjustment, dual publication, three-judge-panel review) -- replaced with the structural argument: minority representation is protected by three features that operate regardless of boundary placement (House expansion lowering effective thresholds, Proportional STAR converting thresholds to seats, blind districting removing manipulation), a guarantee broader than the VRA because it reaches cohesive minorities of every kind rather than only racial and language minorities
- ***Callais* integrated:** Added treatment of *Louisiana v. Callais* as evidence that results-based statutory protection is fragile (Section 2 narrowed to require a strong inference of intentional discrimination; partisan gerrymandering elevated to a permissible criterion even where it coincides with racial lines); framed through the crystal-versus-bricks contrast (targeted legal carve-out dependent on political and judicial weather versus Elections-Clause-grounded structure) and the dissent's "dead letter" and self-liquidation observations; explained why a race-conscious adjustment to a race-blind map would, post-*Callais*, invite Fourteenth Amendment challenge, making the removed carve-out's remedy its own liability
- **New section added:** "What This Does and Does Not Reach" -- federalism passage establishing that the Act governs congressional elections only under the Elections Clause, offers its districting machinery to states for voluntary adoption, leaves state legislative maps to state constitutions, courts, and voters, and reaches only the composition of Congress
- **Incidental carve-out references removed:** Deleted the "narrow VRA adjustment" qualifier from the manipulation/litigation claim (introduction); rewrote the Department of Justice attack-surface defense (Independence Architecture) to reflect that no federal districting role remains to weaponize; removed the "lines touched only when the VRA requires" qualifier from the affirmative-case summary
- **DPS 3.6 alignment confirmed:** Verified against APAI Document Production Standards Rev 3.6; Supporting Document header (title, relational subtitle, author byline) and footer (copyright line) unchanged from Rev 3.4 requirements
...
- **"STAR-PR" renamed to "Proportional STAR":** 4 body instances replaced across proportional representation pairing argument, minority representation section, mutual reinforcement summary, and safe seats analysis
- ESO references reviewed: 2 body references (Tier 3 review process, independence architecture) confirmed accurate under Rev 5.8; no substantive updates required
