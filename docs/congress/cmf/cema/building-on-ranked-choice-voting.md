...
**Revision 6.2**
- No changes required. Rev 6.2 added the affirmative full-field elections standard and split the primary prohibition into two independent bars (no state funding, no state enforcement); this document does not describe the primary-prohibition mechanics in a way affected by the restructure -- it states only that Proportional STAR cuts the primary dependency, making the primary unnecessary rather than load-bearing, which the restructure does not disturb (grep-verified) -- and required no content change. Context guard applied: every "elimination" reference in this document is the ranked-counting mechanic (sequential elimination, elimination rounds, the elimination engine/rule), not "eliminate the primary"; confirmed not a trigger
