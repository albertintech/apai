...
A voting system is monotonic if giving a candidate more support can never cause that candidate to lose. Monotonicity is widely regarded among social choice theorists as a fundamental property -- a minimum standard for a system's internal logic to be coherent. Perversity, as the violation is formally called, means the system can respond backwards to voter preferences: popularity causes defeat, and unpopularity causes victory. The German Federal Constitutional Court struck down a provision of the Federal Elections Act that produced negative voting weight — a phenomenon where more votes for a party could cause it to lose seats — ruling that it violated the constitutional guarantee of equal and direct suffrage (BVerfGE 121, 266). The court's reasoning was specific to Germany's mixed-member proportional system, not to ranked-ballot elimination, but the underlying principle it articulated — that a democratic voting system must not invert the voter's expressed intent — applies with equal force to IRV and STV's non-monotonicity failures, which produce the same category of perverse outcome through a different mechanism.
